# 🎓 Monitoria - UFC Quixadá 🧪

Bem-vindo ao meu repositório de monitoria! Aqui organizo todos os materiais, códigos, listas de exercícios e resoluções que desenvolvo durante minha jornada como monitor na **Universidade Federal do Ceará (UFC) - Campus Quixadá**.

O objetivo deste espaço é centralizar o conhecimento e facilitar o acesso dos alunos aos conteúdos das disciplinas que acompanho.

---

## 🏆 Preparatório OBI (POBI)

Além das disciplinas da graduação, atuei como instrutor para turmas do **POBI (Preparatório para a Olimpíada Brasileira de Informática)** durante os anos de **2025 e 2026**. 

O foco dessas aulas foi o desenvolvimento de lógica avançada, técnicas de programação competitiva e resolução de problemas de alto nível utilizando C++.

---

## 📚 Histórico de Monitoria

Abaixo, as disciplinas que já acompanhei na graduação:

| Disciplina | Sigla | Curso | Conteúdo Principal |
| :--- | :--- | :--- | :--- |
| **Estrutura de Dados** | ED | Sistemas de Informação | Listas encadeadas, Árvores, Grafos e Algoritmos de Ordenação. |
| **Fundamentos de Programação** | FUP | Sistemas de Informação | Lógica de programação, variáveis, laços e funções em C/C++. |
| **Fundamentos de Programação** | FUP | Redes de Computadores | Introdução à lógica e resolução de problemas estruturados. |
| **Prog. Orientada a Objetos** | POO | Redes de Computadores | Classes, Herança, Polimorfismo e Encapsulamento. |

---

## 🛠️ O que você encontra por aqui?

* 📂 **Exercícios Resolvidos:** Respostas detalhadas das listas de exercícios oficiais e desafios de maratona.
* 💻 **Exemplos de Aula:** Códigos implementados durante os horários de monitoria para sanar dúvidas.
* 📝 **Dicas Teóricas:** Pequenos resumos sobre conceitos de programação.
* 🎯 **Preparação para Provas:** Materiais focados nos tópicos mais recorrentes nas avaliações.

---

### 📍 Plataformas Utilizadas
As questões presentes neste repositório foram retiradas e resolvidas das seguintes plataformas:
* **TKO**
* **Moodle**
* **Beecrowd**
* **Neps Academy**

---

## 💬 Contato e Dúvidas

Se você travou em alguma lógica ou encontrou um bug em algum código aqui do repositório, pode me procurar:

* **Discord:** `veriq_`
* **E-mail:** `weryck.lemos@alu.ufc.br`